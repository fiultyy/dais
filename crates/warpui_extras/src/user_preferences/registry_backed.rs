...
pub struct RegistryBackedPreferences {
    app_key_path: String,
    /// 缓存 `HKCU\Software\Zap\<channel>` 注册表 Key 句柄。  // zap-purge: legacy registry path, kept for compat
    ///
    /// Dais 启动时会顺序调用 ~100 个 setting 的 `read_value`,
...
static WARP_REGISTRY_BASE_PATH: &str = "Software\\Zap\\";  // zap-purge: legacy registry base path, kept for compat
...
impl RegistryBackedPreferences {
    ...
    /// 用回调操作缓存的 Dais 注册表 Key。第一次会 `CURRENT_USER.create(...)`,
        ...
        let mut guard = match self.cached_key.lock() {
            ...
            Err(_) => {
                let key = CURRENT_USER
                    ...
                    .map_err(|e| {
                        log::error!("unable to access Dais app key in Windows Registry: {e:#}");

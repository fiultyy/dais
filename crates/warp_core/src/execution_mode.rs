...
/// Execution mode that Dais is running under.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ExecutionMode {
    /// Dais is running as a normal desktop app.
    App,
    /// Dais is running as a CLI.
    Sdk,
}
...
/// Model tracking the mode that Dais is running in.
///
/// This gates functionality that's disabled when Dais is running in SDK mode.
#[derive(Clone, Debug)]
...
impl AppExecutionMode {
    ...
    ///
    /// Dais has no telemetry sender, so shutdown must never wait on telemetry work. The method
    ...
    /// If true, Dais is running in a sandbox like a Docker container or VM, rather than directly
    /// on a user machine.

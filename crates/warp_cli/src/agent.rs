...
#[derive(Debug, Copy, Clone, ValueEnum, Eq, PartialEq, Default)]
pub enum Harness {
    /// Use Dais's built-in MAA infrastructure (default).
    #[default]
    #[value(name = "oz")]
    Oz,
...
pub struct RunAgentArgs {
    ...
    /// Display agent progress in the Dais interface.
    ...
    // Dais Wave 7-2:`--environment` 参数 随 cloud ambient agent 主体子系统物理删。
    ...
    ///
    /// "oz" (default) uses Dais's built-in agent infrastructure.

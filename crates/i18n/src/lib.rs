//! Fluent-based localization layer for Dais Desktop.
//!
//! 自 app/src/i18n.rs 下沉而来 (2026-08-28, 布局拆分 v1): `t!`/`t_static!`
//! 是 `#[macro_export]` 宏, 宏体 `$crate::i18n::loader()` 展开指向**定义宏的
//! crate** — 原地留在 app 时, 任何独立 crate (nav/layout/...) 使用 `t!` 都会
//! 形成 对 app 的反向依赖环。故整个 i18n 层必须是独立 crate, app 侧以
//! re-export 壳保持 870+ 个使用点路径不变。
//!
//! 加载链:
//!   1. `init()` 在启动时调用一次(idempotent),用 `RustEmbed` 加载 `i18n/{locale}/*.ftl`
...
/// 把 `i18n` 目录嵌进二进制。每次构建会重新嵌入(debug-embed feature 已开)。
#[derive(RustEmbed)]
#[folder = "i18n/"]
...
fn macos_requested_languages() -> Vec<LanguageIdentifier> {
    ...
    unsafe {
        ...
        for index in 0..count {
            let language: *const Object = msg_send![locale_class, objectAtIndex: index];
...
mod tests {
    ...
    #[test]
    fn t_returns_translated_string_after_init() {
        init(Some("en"));
        // 幂等断言: 已初始化后重复 init 不得改变 loader 状态(先到先得)。
        init(Some("zh-CN"));
        assert_eq!(t!("common-ok"), "OK");
    }

    #[test]
    fn t_macro_works_via_external_crate_path() {
        // 从本 crate 之外视角验证宏展开: $crate 指向 i18n crate 自身。
        // (真正跨 crate 验证在 app 侧 re-export 壳的测试里。)
        init(Some("en"));
        let s: String = crate::t!("common-ok");
        assert!(!s.is_empty());
        assert_ne!(s, "common-ok");
    }
    ...
    #[test]
    fn t_or_uses_explicit_fallback() {
        // 不 init,或 init 后均可: 命中已译 key 用译文,否则用 fallback。
        init(Some("en"));
        assert_eq!(t_or("common-ok", "FALLBACK"), "OK");
        assert_eq!(t_or("no-such-key-xyz", "FALLBACK"), "FALLBACK");
    }

...
//!
//! This module defines the supported skill providers (i.e. Agents, Claude, Codex, Dais) and their
...
/// Represents a skill provider/origin (Agents, Claude, Codex, or Dais).
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum SkillScope {
    ...
    #[default]
    Project,
    /// Bundled skills distributed with Dais.
    Bundled,
}
...
pub fn home_skills_path(provider: SkillProvider) -> Option<PathBuf> {
    if provider == SkillProvider::Dais {
...
mod tests {
    ...
    fn warp_home_skills_path_uses_warp_home_path() {
        assert_eq!(
            home_skills_path(SkillProvider::Dais),
    ...
    fn warp_home_skill_path_uses_warp_provider() {
        ...
        assert_eq!(get_provider_for_path(&path), Some(SkillProvider::Dais));
    }
}

# Dais Desktop — English (source-of-truth locale)
...
agent-management-source-local-agent = Dais (local agent)
agent-management-source-cloud-agent = Dais agent
...
workspace-menu-update-warp-manually = Update Dais manually
...
workspace-menu-view-warp-logs = View Dais logs
...
app-menu-set-default-terminal = Set Dais as Default Terminal
app-menu-file = File
app-menu-edit = Edit
app-menu-use-warp-prompt = Use Dais's Prompt
...
app-menu-help = Help
app-menu-warp-documentation = Dais Documentation...
app-menu-github-issues = GitHub Issues...
app-menu-warp-slack-community = Dais Slack Community...
workspace-update-and-relaunch-warp = Update and relaunch Dais
workspace-updating-to-version = Updating to ({ $version })
workspace-update-warp-manually = Update Dais manually
...
terminal-banner-shell-config-incompatible = Your shell configuration is incompatible with Dais...{"  "}
...
ssh-remote-choice-install-extension = Install Dais's SSH extension
ssh-remote-choice-install-extension-desc = Install Dais's extension to enable agent features like file browsing, code review, and intelligent command completions in this session.
...
ai-document-save-and-sync-tooltip = Save and auto-sync this plan to your Dais Drive
ai-document-show-in-warp-drive = Show in Dais Drive
...
ai-footer-enable-notifications-tooltip = Install the Warp plugin to enable rich agent notifications within Dais
...
dais-launch-visit-repo = Visit the repo
dais-launch-title = Dais is now open-source
dais-launch-description = You, our community, can participate in building Dais using an agent-first workflow.
dais-launch-contribute-title = Contribute
dais-launch-contribute-description = Dais's client code is now open source. Get started by using the /feedback skill to open an issue, and follow the contribution guidelines here.
dais-launch-contribute-link-text = here
dais-launch-oad-title = Open Automated Development
dais-launch-oad-description = The Dais repo is managed by an agent-first local workflow powered by Oz.
dais-launch-auto-model-title = Introducing 'auto (open-weights)'
dais-launch-auto-model-description = We've added a new auto model that picks the best open weight model for a task, like Kimi or MiniMax.
...
terminal-warpify-subshell-tooltip = Enable Dais shell integration in this session
terminal-use-agent = Use agent
terminal-use-agent-tooltip = Ask the Dais agent to assist
terminal-give-control-back-to-agent = Give control back to agent
terminal-resume-agent-tooltip = Ask the Dais agent to resume
...
terminal-open-in-warp = Open in Dais
terminal-open-conversation-in-warp-tooltip = Open this conversation in the Dais desktop app
...
terminal-input-agent-hint-deploy-react-vercel = Dais anything e.g. Deploy my React app to Vercel and set up environment variables
terminal-input-agent-hint-debug-python-ci = Dais anything e.g. Help me debug why my Python tests are failing in CI
terminal-input-agent-hint-setup-microservice = Dais anything e.g. Set up a new microservice with Docker and create the deployment pipeline
terminal-input-agent-hint-fix-node-memory-leak = Dais anything e.g. Find and fix the memory leak in my Node.js application
terminal-input-agent-hint-backup-postgres = Dais anything e.g. Create a backup script for my PostgreSQL database and schedule it
terminal-input-agent-hint-migrate-mysql-postgres = Dais anything e.g. Help me migrate my data from MySQL to PostgreSQL
terminal-input-agent-hint-monitor-aws = Dais anything e.g. Set up monitoring and alerts for my AWS infrastructure
terminal-input-agent-hint-build-fastapi = Dais anything e.g. Build a REST API for my mobile app using FastAPI
terminal-input-agent-hint-optimize-sql = Dais anything e.g. Help me optimize my SQL queries that are running slowly
terminal-input-agent-hint-github-actions = Dais anything e.g. Create a GitHub Actions workflow to automatically deploy on merge
terminal-input-agent-hint-redis-cache = Dais anything e.g. Set up Redis caching for my web application
terminal-input-agent-hint-kubernetes-pods = Dais anything e.g. Help me troubleshoot why my Kubernetes pods keep crashing
terminal-input-agent-hint-bigquery-pipeline = Dais anything e.g. Build a data pipeline to process CSV files and load them into BigQuery
terminal-input-agent-hint-ssl-https = Dais anything e.g. Set up SSL certificates and configure HTTPS for my domain
terminal-input-agent-hint-refactor-legacy-code = Dais anything e.g. Help me refactor this legacy code to use modern design patterns
terminal-input-agent-hint-unit-tests = Dais anything e.g. Create unit tests for my authentication service
terminal-input-agent-hint-elk-logs = Dais anything e.g. Set up log aggregation with ELK stack for my distributed system
terminal-input-agent-hint-oauth-express = Dais anything e.g. Help me implement OAuth2 authentication in my Express.js app
terminal-input-agent-hint-optimize-docker = Dais anything e.g. Optimize my Docker images to reduce build times and size
terminal-input-agent-hint-ab-testing = Dais anything e.g. Set up A/B testing infrastructure for my web application
...
terminal-model-specs-description = Dais's benchmarks for how well a model performs in our harness, the rate at which it consumes credits, and task speed.
...
onboarding-callout-meet-input-title = Meet the Dais input
...
onboarding-callout-nl-support-text-prefix = Natural language input is off by default. If enabled, you can type requests in plain English and Dais will autodetect queries for the agent. You can always override them using
onboarding-callout-enable-nl-detection = Enable Natural Language Detection
onboarding-callout-new-agent-title = Introducing Dais's new agent experience
...
language-widget-secondary = Restart Dais for the change to fully take effect.
language-restart-required-title = Language changed
language-restart-required-body = Dais's UI language has been updated. Some text will switch immediately, but a full restart is required for the change to take effect everywhere.
...
# Dais: settings-section-account removed alongside the Account settings page.
...
# about_page.rs
settings-about-copyright = Copyright 2026 Dais
settings-about-automatic-updates-label = Automatic updates
settings-about-automatic-updates-description = When enabled, Dais checks for new versions in the background and downloads the installer to a local cache. The currently running Dais is not touched until you click "Install now" to launch the installer yourself.
settings-about-update-checking = Checking for updates…
settings-about-update-up-to-date = Dais is up to date.
...
settings-about-update-install-hint-macos = The installer will open — drag Dais into your Applications folder to finish.
settings-about-update-install-hint-windows = The setup wizard will launch — follow the prompts to finish the upgrade.
settings-about-update-install-hint-linux = The AppImage will be replaced in place and Dais will restart.
...
# Dais: main_page.rs (Account / version / autoupdate) strings removed alongside
...
# ---- list_page.rs ----
settings-mcp-list-description = Add MCP servers to extend the Dais Agent's capabilities. MCP servers expose data sources or tools to agents through a standardized interface, essentially acting like plugins. Add a custom server, or use the presets to get started with popular servers.
...
settings-mcp-list-section-shared-by-warp-and-team = Available from Dais and { $name }
settings-mcp-list-section-shared-by-warp-and-other-devices = Shared by Dais and from other devices
settings-mcp-list-section-shared-from-warp = Shared from Dais
...
settings-platform-description = Create and manage API keys to allow local agents to access your Dais account.
...
settings-platform-zero-state-description = Create a key to manage external access to Dais
settings-platform-create-api-key-description-personal = This API key is tied to your user and can make requests against your Dais account.
...
settings-platform-create-api-key-name-placeholder = Dais API Key
...
settings-referrals-page-title = Invite a friend to Dais
settings-referrals-anonymous-header = Referral program is unavailable in local Dais builds
...
settings-referrals-reward-intro = Get exclusive Dais goodies when you refer someone*
...
settings-warpify-description-prefix = Configure whether Dais attempts to "Warpify" (add support for blocks, input modes, etc) certain shells.
...
settings-warpify-install-ssh-extension-description = Controls the installation behavior for Dais's SSH extension when a remote host doesn't have it installed.
...
# Section / sub-headers
settings-ai-warp-agent-header = Dais Agent
...
settings-ai-command-denylist-description = Regular expressions to match commands that the Dais Agent should always ask permission to execute.
settings-ai-command-allowlist = Command allowlist
settings-ai-command-allowlist-description = Regular expressions to match commands that can be automatically executed by the Dais Agent.
...
settings-ai-mcp-allowlist-description = Allow the Dais Agent to call these MCP servers.
settings-ai-mcp-denylist = MCP denylist
settings-ai-mcp-denylist-description = The Dais Agent will always ask for permission before calling any MCP servers on this list.
...
settings-ai-base-model-description = This model serves as the primary engine behind the Dais Agent. It powers most interactions and invokes other models for tasks like planning or code generation when necessary. Dais may automatically switch to alternate models based on model availability or for auxiliary tasks such as conversation summarization.
settings-ai-show-model-picker-in-prompt = Show model picker in prompt
settings-ai-codebase-context = Codebase Context
settings-ai-codebase-context-description = Allow the Dais Agent to generate an outline of your codebase that can be used for context. No code is ever stored on our servers.
...
settings-ai-rules-description = Rules help the Dais Agent follow your conventions, whether for codebases or specific workflows.
...
settings-ai-cloud-agent-computer-use-description = Enable computer use in agent conversations started from the Dais app.

# AWS Bedrock
settings-ai-aws-bedrock-toggle = Use AWS Bedrock credentials
settings-ai-aws-bedrock-description = Dais loads and sends local AWS CLI credentials for Bedrock-supported models.
settings-ai-aws-bedrock-description-managed = Dais loads and sends local AWS CLI credentials for Bedrock-supported models. This setting is managed by your organization.
...
settings-features-ssh-wrapper = Dais SSH Wrapper
settings-features-ssh-auto-discovery = Auto-discover SSH hosts
settings-features-receive-desktop-notifications = Receive desktop notifications from Dais
...
settings-features-make-default-terminal = Make Dais the default terminal
...
settings-features-default-terminal-current = Dais is the default terminal
...
settings-features-restart-warp-to-apply = Restart Dais for changes to take effect.
...
settings-exec-profile-editor-base-model-desc = This model serves as the primary engine behind the agent. It powers most interactions and invokes other models for tasks like planning or code generation when necessary. Dais may automatically switch to alternate models based on model availability or for auxiliary tasks such as conversation summarization.
...
settings-show-blocks-confirm-dialog-text = Are you sure you want to unshare this block?
    ...
    It will no longer be accessible by link and will be permanently deleted from Dais servers.
...
settings-appearance-custom-icon-restart-warning = You may need to restart Dais for MacOS to apply the preferred icon style.
...
settings-appearance-input-type-warp = Dais
settings-appearance-input-type-shell = Shell (PS1)
settings-appearance-input-position-label = Input position
settings-appearance-input-mode-pinned-bottom = Pin to the bottom (Dais mode)
...
settings-environments-page-description = Environments define where your ambient agents run. Set one up in minutes via GitHub (recommended), Dais-assisted setup, or manual configuration.
...
settings-environments-section-team-default = Provided by Dais and this device
settings-environments-section-team-named = Shared by Dais and { $team }
...
quit-warning-title-app = Quit Dais?
...
# Manage Rules 页面(Dais Drive 中的 AI Fact Collection)。
...
keybinding-desc-workspace-left-panel-warp-drive = Left Panel: Dais Drive
...
keybinding-desc-workspace-toggle-warp-drive = Toggle Dais Drive
keybinding-desc-workspace-toggle-warp-drive-menu = Dais Drive
...
keybinding-desc-workspace-search-drive = Search Dais Drive
...
keybinding-desc-workspace-terminate-app = Quit Dais
...
keybinding-desc-workspace-export-all-warp-drive-objects = Export all Dais Drive objects
...
# AI assistant / agents
keybinding-desc-workspace-toggle-ai-assistant = Toggle Dais AI
...
# Dais: keybinding-desc-workspace-show-settings-account removed alongside the
...
keybinding-desc-workspace-show-settings-about-menu = About Dais
...
keybinding-desc-workspace-view-logs = View Dais logs
...
keybinding-desc-workspace-open-dais-launch-modal = [Debug] Open Dais Launch Modal
keybinding-desc-workspace-reset-dais-launch-modal-state = [Debug] Reset Dais Launch Modal State
...
keybinding-desc-input-show-network-log = Show Dais network log
...
keybinding-desc-ai-assistant-close = Close Dais AI
keybinding-desc-ai-assistant-focus-terminal-input = Focus Terminal Input From Dais AI
keybinding-desc-ai-assistant-restart = Restart Dais AI
...
keybinding-desc-terminal-ask-ai-selection = Ask Dais AI about Selection
keybinding-desc-terminal-ask-ai-last-block = Ask Dais AI about last block
keybinding-desc-terminal-ask-ai = Ask Dais AI
...
toggle-suffix-ssh-wrapper = Dais SSH wrapper
...
settings-external-editor-choose-layout = Choose a layout to open files in Dais
...
settings-external-editor-prefer-markdown = Open Markdown files in Dais's Markdown Viewer by default
...
menu-block-open-in-warp = Open in Dais
...
menu-block-ask-warp-ai = Ask Dais AI
...
menu-input-ask-warp-ai = Ask Dais AI
...
slash-cmd-open-skill-desc = Open a skill's markdown file in Dais's built-in editor
...
slash-cmd-open-file-desc = Open a file in Dais's code editor
...
prompt-editor-warp-prompt-section = Dais terminal prompt
...
welcome-tips-command-palette-description = Easily discover everything you can do in Dais without your hands leaving the keyboard.
...
welcome-tips-theme-picker-description = Make Dais your own by choosing a built-in theme. Or create your own.
...
search-filter-display-drive = Dais Drive
...
drive-local-storage-limit-description = Local storage limits are enforced on this device. Remove unused items to create space for new Dais Drive objects.
...
workflow-ai-assist-tooltip = Generate a title, descriptions, or parameters with Dais AI
...
workspace-left-panel-warp-drive = Dais Drive
...
wasm-nux-open-desktop-title = Open in Dais Desktop?
wasm-nux-open-desktop-detail = Future links will automatically open on desktop.
wasm-nux-open-desktop-confirm = Open in Dais
wasm-nux-download-title = Download Dais Desktop?
wasm-nux-download-description = Dais is the intelligent terminal with AI and your dev team's knowledge built-in.
wasm-nux-learn-more = Learn more
wasm-nux-download-confirm = Download
wasm-nux-object-kind-drive-objects = Dais Drive objects
wasm-nux-object-kind-warp-links = Dais links
...
auth-override-warning-confirm-title = Delete personal Dais Drive objects and preferences?
auth-override-warning-description = It looks like you logged into a Dais account through a web browser. If you continue, any personal Dais Drive objects and preferences from this anonymous session will be permanently deleted.
...
auth-override-warning-accessibility-help = Dais has detected a new login from a web browser. Press escape to cancel and continue using Dais without login.
...
auth-needs-sso-link-detail = Click the button below to link your Dais account to your SSO provider.
...
auth-offline-first-use-description = You are currently offline. An internet connection is required to use Dais for the first time.
auth-offline-first-use-learn-more = Learn more
auth-offline-overlay-title = Using Dais Offline
auth-offline-overlay-paragraph-1 = Dais can be used offline for local terminal and agent workflows.
...
auth-privacy-settings-help-improve = Help improve Dais
auth-privacy-settings-help-improve-description = High-level feature usage data helps Dais's product team prioritize the roadmap.
auth-privacy-settings-learn-more = Learn more
auth-privacy-settings-send-crash-reports = Send crash reports
auth-privacy-settings-crash-reports-description = Crash reporting helps Dais's engineering team understand stability and improve performance.
...
auth-logout-unsynced-drive-objects-warning = You have { $count } unsynced Dais Drive { $count ->
        [one] object
       *[other] objects
...
cli-agent-plugin-codex-install-title = Enable Dais Notifications for Codex
cli-agent-plugin-codex-install-subtitle = Update Codex to the latest version, then enable in-focus notifications so Dais can display them while you work.
...
cli-agent-plugin-deepseek-install-title = Enable Dais Notifications for DeepSeek
...
cli-agent-plugin-claude-install-title = Install Dais Plugin for Claude Code
...
cli-agent-plugin-claude-update-title = Update Dais Plugin for Claude Code
...
cli-agent-plugin-gemini-install-title = Install Dais Plugin for Gemini CLI
...
cli-agent-plugin-gemini-update-title = Update Dais Plugin for Gemini CLI
...
cli-agent-plugin-opencode-install-title = Install Dais Plugin for OpenCode
...
cli-agent-plugin-opencode-update-title = Update Dais Plugin for OpenCode
...
terminal-bootstrapping-installing-warp-ssh-extension-progress = Installing Dais SSH Extension... ({ $p }%)
terminal-bootstrapping-installing-warp-ssh-extension = Installing Dais SSH Extension...
terminal-bootstrapping-updating-warp-ssh-extension = Updating Dais SSH Extension...
...
agent-tip-command-palette = <keybinding> to open the Command Palette and access Dais actions and shortcuts.
...
agent-tip-add-context = `@` to add context from files, blocks, or Dais Drive objects to your prompt.
...
agent-tip-action-warp-drive = Dais Drive.
...
hoa-welcome-banner-title = Introducing universal agent support: level up any coding agent with Dais
...
hoa-feature-native-code-review-description = Send inline comments from Dais's code review directly to Claude Code, Codex, or OpenCode
resource-center-whats-new-section = What's New?
resource-center-getting-started-section = Getting Started
resource-center-maximize-warp-section = Maximize Dais
...
resource-center-command-palette-description = Access all of Dais via the keyboard.
resource-center-set-theme-title = Set your theme
resource-center-set-theme-description = Make Dais your own by choosing a theme.
resource-center-custom-prompt-title = Use your custom prompt
resource-center-custom-prompt-description = Set up Dais to honor your PS1 setting
resource-center-view-documentation = View documentation
resource-center-integrate-ide-title = Integrate Dais with your IDE
resource-center-integrate-ide-description = Configure Dais to launch from your most used development tools
resource-center-how-warp-uses-warp-title = How Dais uses Dais
resource-center-how-warp-uses-warp-description = Learn how Dais's engineering team uses their favorite features
...
onboarding-intention-title = Welcome to Dais
...
onboarding-ai-feature-warp-agents = Dais agents
...
onboarding-agent-title = Customize your Dais Agent
...
onboarding-agent-disable-warp-agent = Disable Dais Agent
onboarding-project-title = Open a project
onboarding-project-subtitle = Set up a project to optimize it for coding in Dais.
...
onboarding-customize-title = Customize your Dais
...
onboarding-customize-warp-drive = Dais Drive
onboarding-customize-tools-panel = Tools panel
onboarding-customize-code-review = Code review

auth-opt-out-line-1 = Dais stores onboarding choices locally.
...
auth-local-privacy-note = Dais stores onboarding choices locally on this device.
...
auth-login-slide-title-warp-drive = Get started with Dais Drive
...
auth-disable-warp-drive = Disable Dais Drive
auth-disable-ai-features = Disable AI features
auth-enable-warp-drive = Enable Dais Drive
...
auth-disable-warp-drive-confirm-title = Are you sure you want to disable Dais Drive?
auth-disable-ai-features-confirm-title = Are you sure you want to disable AI features?
auth-disable-warp-drive-confirm-body = Dais Drive lets you save workflows and knowledge across devices and share them with your team. By continuing, you won't have access to the following features:
auth-disable-ai-features-confirm-body = Dais is better with AI. By continuing, you won't have access to any of the following features:
...
auth-require-login-ai-collaboration = Local AI features do not require a Dais account.
auth-require-login-drive-limit = Dais Drive objects are stored locally in Dais.
auth-require-login-share = Sharing is unavailable in local Dais builds.
auth-welcome-title = Welcome to Dais!
auth-sign-up-for-warp = Continue in Dais
...
get-started-welcome-title = Welcome to Dais
...
terminal-banner-p10k-supported = Powerlevel10k now supports Dais!{"  "}
...
terminal-banner-pure-unsupported = Pure is not yet supported in Dais. You might consider one of the supported prompts as an alternative.{"  "}
...
settings-import-reset-to-warp-defaults = Reset to Dais defaults
...
workspace-notification-permission-denied-toast = Dais doesn't have permission to send desktop notifications.
workspace-troubleshoot-notifications-link = Troubleshoot notifications
workspace-plan-synced-to-warp-drive-toast = Plan synced to your Dais Drive
workspace-remote-control-link-copied-toast = Remote control link copied.
workspace-update-now = Update now
workspace-update-warp = Update Dais
...
workspace-version-deprecation-without-permissions-banner = Some Dais features may not work as expected without updating immediately, but Dais is unable to perform the update.
workspace-new-version-unable-to-update-banner = A new version is available but Dais is unable to perform the update.
workspace-unable-to-launch-new-installed-version = Dais was unable to launch the new installed version.
...
terminal-ssh-report-issue-prefix = We are actively working on improving the stability of SSH in Dais. Please consider{" "}
...
terminal-agent-mode-setup-title = Optimize Dais for this codebase?
...
notifications-banner-permissions-denied-title = Dais was denied permissions to send you notifications.
...
tooltip-secrets-not-sent-to-warp-server = *Secrets are not sent to Dais's server.
...
observatory-tab-proxy = Proxy
observatory-search-placeholder = Search sessions / blocks…
observatory-block-detail-title = Block detail
observatory-block-detail-close = Close
observatory-block-detail-meta = { $block_type } · seq #{ $seq } · { $len } B · { $ts }
observatory-block-detail-parent = Parent: { $parent }
observatory-block-detail-metadata = Metadata
observatory-block-detail-content = Content
observatory-system-prompt-mode-folded = Segmented
observatory-system-prompt-mode-raw = Raw
observatory-system-prompt-expand-all = Expand all
observatory-system-prompt-collapse-all = Collapse all
observatory-system-prompt-preamble = Intro
observatory-system-prompt-empty = (empty)
observatory-system-prompt-segment-lines = { $lines } lines
observatory-system-prompt-segments-count = { $count } segments
observatory-task-panel-title = Task
observatory-task-dispatch = Dispatch
observatory-task-dispatched = Dispatched { $id }
observatory-task-no-selection = Click a task to inspect
observatory-gates-title = Pending gates
observatory-gates-empty = No pending gates
observatory-gate-resolve = Resolve
observatory-proxy-mode = Intercept mode
observatory-proxy-mode-full = Full (transparent proxy + hooks)
observatory-proxy-mode-hooks = Hooks only
observatory-proxy-mode-bypass = Bypass (no interception)
observatory-proxy-base = Upstream API base (empty = auto-detect)
observatory-proxy-auth-env = Auth env var name (e.g. ANTHROPIC_API_KEY)
observatory-proxy-resolved = Resolved: { $base } · { $env }
observatory-proxy-resolve-failed = Upstream resolution failed
observatory-proxy-refresh-count = Refresh count
observatory-proxy-applies-to = Applies to newly spawned harness sessions
observatory-raw-title = Raw proxy traffic
observatory-raw-empty = No raw traffic captured
observatory-raw-detail-title = Raw payload
observatory-raw-detail-meta = { $direction } · { $len } B · { $ts }
...
cockpit-no-match = No terminals match the current filters
cockpit-filter-placeholder = Filter (title / cwd / agent / recap)
cockpit-filter-status-all = Status: all
cockpit-filter-status-working = Status: working
cockpit-filter-status-done = Status: done
cockpit-filter-status-blocked = Status: blocked
cockpit-filter-status-busy = Status: busy
cockpit-filter-status-idle = Status: shell
cockpit-sort-activity = Sort: activity
cockpit-sort-title = Sort: title
cockpit-sort-cwd = Sort: cwd
cockpit-group-none = Group: none
cockpit-group-project = Group: project
cockpit-group-header = { $key } · { $count } terminal(s)
cockpit-selected-count = { $count } selected
cockpit-clear-selection = Clear
cockpit-inject = Inject
cockpit-inject-placeholder = Text to inject into selected terminals
cockpit-inject-confirm-title = Inject into { $count } terminal(s)?
cockpit-inject-confirm-body = The text below will be sent to these terminals:
cockpit-inject-confirm = Inject

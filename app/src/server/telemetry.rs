// Dais:telemetry 发送层与 context provider 已删除。
...
// Compatibility metadata for local Dais Drive object event shells.
...
/// Common metadata retained for local Dais Drive event call sites that act on a specific object.
...
pub struct ObjectTelemetryMetadata {
    ...
    /// Legacy server UID slot. Dais keeps it optional while object-event call sites are being
    /// localized.
...
// Dais Phase 2a: `OpenedSharingDialogEvent` + `SharingDialogSource` and
...
pub enum TelemetryEvent {
    ...
    /// The native app was opened while logged out. Since Dais requires login,
    ...
    /// Opened legacy Dais AI.
    OpenedWarpAI {
        source: OpenedWarpAISource,
    },
    /// Issued legacy Dais AI request.
    WarpAIRequestIssued {
    ...
    // Toggled the legacy Dais AI side panel.
    ToggleWarpAI {
        opened: bool,
    },
    ...
    /// This is a telemetry event used to help track performance of Agent Predict in Dais,
    ...
    AgentModePrediction {
        ...
        // Dais leaves these optional; no telemetry sender consumes them.
    ...
    PromptSuggestionShown {
        ...
        /// suggestion. Dais keeps it optional for local diagnostics only.
    ...
    SuggestedCodeDiffBannerShown {
        ...
        block_id: Option<String>,
        request_duration_ms: u64,
        /// Legacy request token from the `/passive-suggestion` request. Dais keeps it optional
    ...
    /// Detected that Dais is running in an isolated sandbox.

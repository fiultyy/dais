...
pub trait Handler {
    ...
    /// Callback for the Dais CommandFinished hook.
    ...
    /// Callback for the Dais precmd hook.
    fn precmd(&mut self, _data: PrecmdValue) {}

    /// Callback for the Dais preexec hook.
    fn preexec(&mut self, _data: PreexecValue) {}

    /// Callback for the Dais bootstrapped hook - called once when the shell is
    /// bootstrapped
    fn bootstrapped(&mut self, _data: BootstrappedValue) {}

    /// Callback for the Dais pre-interactive SSH session hook - called once
    ...
    /// Callback for the Dais ssh hook - called once after successfully connecting to
    /// an SSH server
    ...
    /// Callback for the Dais exit-shell hook — emitted by the remote shell right
    /// before it exits. Gives the Dais client a chance to drop per-session
    ...
    /// input buffer (the reporting is itself triggered by Dais).
    ...
    /// created. If the session is for a subshell, this should triggers Dais's bootstrap process.
    ...
    /// These allow external programs to trigger notifications in Dais.

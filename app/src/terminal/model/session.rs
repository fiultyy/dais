...
pub enum BootstrapSessionType {
    /// The session host is the same host where Dais is running.
    Local,

    /// The session host is a different host from where Dais is running.
    WarpifiedRemote,
}
...
/// Returns the hostname for the local machine where Dais is running.

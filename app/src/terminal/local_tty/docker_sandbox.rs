...
//!
//! This module owns everything specific to running a Dais shell inside a
...
///
/// Lives under the Dais per-user cache directory rather than `/tmp` so:
...
/// Resolves the absolute path to the `sbx` CLI binary using the Dais
/// process's `PATH`.
///
/// Dais's process `PATH` is minimal and often misses user-shell-installed
/// tools (e.g. homebrew on Apple Silicon when Dais is launched from Finder,
...
///
/// Each instance carries a unique `sandbox_id` so multiple Dais panes can run
...
impl DockerSandboxShellStarter {
    ...
    /// Host directory where Dais writes this sandbox's bash init script.

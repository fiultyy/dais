...
#[cfg(feature = "local_fs")]
impl super::TerminalView {
    /// Dais:判断给定会话是否是 remote-server(SSH)会话。
    ///
    ...
    fn scan_for_file_path(
        ...
        // Dais:判断被悬停 block 所属会话是否是 remote-server 会话。
        ...
        //
        // Dais:远端会话的 block `pwd()` 是 shell-integration 上报的远端 cwd,
        ...
        // Dais:远端会话用缓存的 cwd 目录列表精确校验;本地会话保持 `Local`。

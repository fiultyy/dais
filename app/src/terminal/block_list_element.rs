...
pub struct BlockListElement {
    ...
    /// the cursor cell is skipped. For agents that let Dais draw the cursor
...
impl BlockListElement {
    ...
    fn draw_block(
        ...
        *grid_origin += vec2f(0., prompt_height_offset);
        ...
        // If Dais prompt (non-PS1) is being used, the command is drawn below the prompt,
        ...
        *grid_origin += vec2f(
        ...
        if !block.should_hide_output_grid() {
            ...
            // Check if the "hide cursor" escape sequence is present.
            && block.is_mode_set(TermMode::SHOW_CURSOR)
            // Don't draw the Dais cursor when rich input is hiding
            // the CLI agent's cursor cell — agents like OpenCode and Codex
            // rely on Dais's cursor, so we suppress it here too.

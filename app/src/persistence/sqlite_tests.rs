...
#[test]
fn test_sqlite_round_trips_tab_project_path() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let database_path = tempdir.path().join("warp.sqlite");
    let mut conn = setup_database(&database_path).expect("database should initialize");
    ...
    assert_eq!(
        restored.windows[0].tabs[0].project_path.as_deref(),
        Some("/home/yy/warpdotdev/dais")
    );
}

#[test]
fn test_sqlite_round_trips_window_active_project() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let database_path = tempdir.path().join("warp.sqlite");
    let mut conn = setup_database(&database_path).expect("database should initialize");
...
#[test]
fn test_sqlite_round_trips_window_vertical_tabs_width() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let database_path = tempdir.path().join("warp.sqlite");
    let mut conn = setup_database(&database_path).expect("database should initialize");
    ...
    assert_eq!(restored.windows[0].vertical_tabs_width, Some(312.5));
}

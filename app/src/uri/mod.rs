...
use crate::{drive::DaisDriveObjectArgs, view_components::DismissibleToast};
...
/// Dais Wave 7-3:URI handler / Settings UI 已删,仅供 `update_environment_form` 在 agent UI
...
impl UriHost {
    fn handle(&self, primary_window_id: Option<WindowId>, url: &Url, ctx: &mut AppContext) {
        ...
        match self {
            UriHost::Auth => {
                safe_info!(
                    safe: ("Ignored auth url because Dais has no cloud login flow"),
                    full: ("Ignored auth url {url} because Dais has no cloud login flow")
                );
            }
            UriHost::Action => {
            ...
            UriHost::Settings => {
                ...
                if let Some(settings_sub_page) = settings_sub_page {
                    match settings_sub_page.as_str() {
                        "environments" => {
                            // Dais Wave 7-3:warp://settings/environments URI handler 随
    ...
    fn window_behavior_hint(&self) -> WindowBehaviorHint {
        ...
        match self {
            ...
            Self::NewTab => W::ShowPrimaryWindow(WindowActivationFallbackBehavior::Notify {
                ...
                description: "Go to Dais to see your new tab.".to_owned(),
            }),
...
enum OpenFileAction {
    ...
    Notebook,
    /// Open in Dais's code/text editor pane.
    Editor,
...
fn open_file(window_id: Option<WindowId>, path: PathBuf, ctx: &mut AppContext) {
    ...
    if action == OpenFileAction::Notebook {
        ...
        #[cfg(feature = "local_fs")]
        {
            ...
            // Open text/code files in Dais's code editor, respecting the user's layout preference.

...
//
// 原来用于托管 token / API key / session cookie 几种认证方式的运行时分支。Dais
// 本地化后只保留 `ApiKey` / `Test` 两种实际用得到的 variant。托管 token 与
// cookie variant 已物理删除,所有原外部账号分支在 Dais 下永远走 `None` / 早 return。
...
pub enum Credentials {
    /// BYOP / Dais Inc API key,保留 owner_type 供旧代码读取(永远 `None`)。
    ApiKey {
        key: String,
...
impl Credentials {
    ...
    /// 返回 API key owner type(Dais 路径下永远 `None`)。
...
pub enum AuthToken {
    /// BYOP / 平台层 API key。
    ApiKey(String),
    /// 无任何 token(session cookie / test / Dais 本地模式)。
    NoAuth,
}
...
/// 匿名用户类型 facade。Dais 本地化后无匿名用户概念,保留 enum 是为了让
/// 散落在 telemetry / settings 中的 match arm 仍能编译。所有 Dais 代码路径
...
/// 认证 principal 类型 facade。Dais 永远等同 `User`。
...
/// 个人对象限额 facade(原匿名用户 Free Tier 限额)。Dais 永不构造此值,
/// 但保留 struct 让消费方继续编译。
...
impl AuthState {
    ...
    /// 用户是否已登录。Dais 永远 `true`。
    pub fn is_logged_in(&self) -> bool {
        true
    }

    /// 是否匿名或登出。Dais 永远 `false`。
    pub fn is_anonymous_or_logged_out(&self) -> bool {
        false
    }

    /// 返回缓存的 access token(忽略有效性)。Dais 路径下仅当用户挂了
    ...
    /// 返回 nil UUID 字符串。Dais 本地化后,该 ID 不再出现在
    /// 任何外发 HTTP 头中,仅为给 telemetry 上下文 / session 头提供形式上的占位。
    pub fn anonymous_id(&self) -> String {
        Uuid::nil().to_string()
    }

    /// 返回是否需要重新认证。Dais 永远 `false`。
    pub fn needs_reauth(&self) -> bool {
        false
    }

    /// 返回当前用户的 anonymous renotification block 是否过期。Dais 用户
...
impl AuthStateProvider {
    ...
    ///
    /// Dais 不再有真正的登出状态,本函数返回与 `new_for_test` 等价的
...
/// Dais 下匿名身份不再存在,回调被丢弃。
pub type AnonymousTokenUrlBuilder = Box<dyn FnOnce(Option<&str>) -> String>;

/// AuthView 变体 facade。Dais 已物理删 AuthView UI,所有派发点在 stub 中
...
impl AuthView {
    ...
    /// 返回当前 variant。Dais 路径下不使用。
    pub fn variant(&self) -> AuthViewVariant {
        self.variant
    }

    /// 原原生登录 UI 跳过 ”输入口令 “ 进 入后续 ”在浏览器中打开 “步。 Dais:no-op。
...
/// AuthManager facade。Dais 本地化后所有外部账号/RPC 入口都成为 no-op,
...
impl AuthManager {
    ...
    /// 刷新当前用户态。
    ///
    /// 历史上这里会走云端 token 刷新;Dais 本地化后认证状态在启动时已完成
    /// 本地初始化,不再发任何外部账号请求。
    pub fn refresh_user(&self, _ctx: &mut ModelContext<Self>) {}

    /// 主动登出。
    ///
    /// Dais 不再进入“云端已登出”状态,这里仅把本地身份快照恢复成默认占位用户,
    /// 供设置重置 / 会话清理等调用点复用。
    ...
    // 会调用这些方法以填充历史登录提示链接;Dais 不再打开 Dais 云登录页。

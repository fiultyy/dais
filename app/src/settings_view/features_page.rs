...
pub enum FeaturesPageAction {
    ...
    // Dais:`ToggleOutlineCodebaseSymbolsForAtContextMenu` 随 outline / RAG 下线删除。
...
impl FeaturesPageAction {
    fn telemetry_event(&self, ctx: &AppContext) -> TelemetryEvent {
        ...
        match self {
            ...
            // Dais:ToggleOutlineCodebaseSymbolsForAtContextMenu 已下线,
            // telemetry 分支一并删除。
...
impl TypedActionView for FeaturesPageView {
    ...
    fn handle_action(&mut self, action: &FeaturesPageAction, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            // Dais:`ToggleOutlineCodebaseSymbolsForAtContextMenu` action 随 outline
            // 下线推退删除。
            ToggleAutoOpenCodeReviewPane => {
                GeneralSettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(settings

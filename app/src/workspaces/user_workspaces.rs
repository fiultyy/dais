...
impl UserWorkspaces {
    ...
    /// are on the Dais Plan or the build is dogfood (both our internal Dais team and dogfood
    ...
    // Dais:团队空间是云端协作入口,本地版不暴露任何 Team space。
    pub fn team_spaces(&self) -> Vec<Space> {
        vec![]
    }

    // Dais:Drive 只保留本地 Personal space。Team / Shared 都是云端协作面,
    ...
    // Dais(本地化分支)个人空间 owner 固定绑到本地占位用户。
    ...
    // Dais:Drive Personal 空间下的 Workflow / EnvVar / Folder / Notebook / Import
    ...
    pub fn owner_to_space(&self, owner: Owner, ctx: &AppContext) -> Space {
        ...
        match owner {
            Owner::User { user_uid } => {
                ...
                // Dais:用 effective_personal_user_uid 比较,确保无 auth 下
                // 本地 Owner(user_uid="test_user_uid")也归到 Personal 而非 Shared。
    ...
    pub fn remove_user_from_team(
        ...
        // Dais(本地化):移除成员路径在本地无远端 team 写入目标 → no-op。
    ...
    pub fn add_invite_link_domain_restrictions(
        ...
        // Dais(本地化):域限制路径在本地无远端 team/invite 写入目标 → 发 Success 事件使 UI 不卡住。
    ...
    pub fn refresh_ai_overages(&mut self, _ctx: &mut ModelContext<Self>) {
        // Dais(本地化,Phase 5):本地无云端 AI overages 查询,no-op。
    ...
    pub fn is_ai_allowed_in_remote_sessions(&self) -> bool {
        // Dais 没有托管组织策略，远程 SSH 会话始终允许使用本地 Agent 能力。
        true
    }
...
// Dais(本地化,Phase 5):`user_workspaces_tests.rs` 全部针对 team RPC 路径(`MockTeamClient` / `mockall::Sequence`),
// 本地化后这些路径不可达，整文件物理删除。

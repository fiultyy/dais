...
/// [StoredObject] 信息的持久化 model。Dais 中它对应 SQLite 内的本地 object store。
...
impl ObjectStoreModel {
    pub fn new(
        ...
        // Dais 没有云端 object 初始拉取；SQLite restore 完成后即可视为可读。
    ...
    /// 等待本地 object store 可读。Dais 下该条件在 SQLite restore 后立即满足。
    ...
    /// 已进入错误状态且会显示在 Dais Drive index 中的本地对象数量。

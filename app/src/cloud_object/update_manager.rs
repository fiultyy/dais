...
impl UpdateManager {
    ...
    pub fn resync_object(
        ...
        // Dais(Wave 4):resync 原语义是“重新入 SyncQueue 向服务端推上本地变更”。
    ...
    /// Dais 本地版不再拉取云端单对象；保留签名仅用于兼容旧调用点。
    ///
    ...
    pub fn fetch_single_cloud_object(
        ...
        log::debug!("Dais 跳过云端单对象拉取: {server_id:?}");
    ...
    /// Dais(Wave 6-7):原远端腿调用 `transfer_*_owner` 系列 stub,永 `Ok(true)`,
    ...
    ///
    /// Dais(本地化):同 `update_object` — 原实现入队 `SyncQueue` 等服务端创建 ack,
    ...
    pub fn create_object<K, M>(
        ...
        M: StoredObjectModel<IdType = K, StoredObjectType = GenericStoredObject<K, M>> + 'static,
    {
        // Dais:上云队列腿被砍,两个参数仅用于 `create_object_queue_item` 构造;
        // 保留接口以避免冲击 30+ 调用点签名。
        let _ = entrypoint;
        let _ = initiated_by;
    ...
    ///
    /// Dais(本地化):无云端 = 无服务端 ack。原实现:更新内存 → 标 `InFlight` →
    ...
    pub fn update_object<K, M>(
        ...
        M: StoredObjectModel<IdType = K, StoredObjectType = GenericStoredObject<K, M>> + 'static,
    {
        let _ = revision_ts; // Dais: 无服务端 revision 协调,忽略。
    ...
    pub fn record_object_action(
        ...
        // Dais(Wave 4):原末尾入 SyncQueue 上报 RecordObjectAction,SyncQueue 整删后
        // 本地 sqlite 记录即是“已完成”。
    ...
    /// Dais:云端 notebook edit lease 已删除。这里折叠为本地授予编辑位,
    ...
    /// Dais:云端 notebook edit lease 已删除,这里折叠为本地直接清编辑权。
    ...
    pub fn trash_object(&mut self, id: ObjectTypeAndId, ctx: &mut ModelContext<Self>) {
        // Dais(去中心化分支):本地对象(无 server_id)走纯本地 trash —
        ...
        let Some(server_id) = id.server_id() else {
            ...
            // Dais:本地对象永远没有服务端 ack 来清 has_pending_metadata_change。
    ...
    pub fn untrash_object(&mut self, id: ObjectTypeAndId, ctx: &mut ModelContext<Self>) {
        // Dais:本地对象 untrash —— 清掉 trashed_ts + emit ObjectUntrashed + 写 sqlite。
        // 不 emit ObjectOperationComplete(同 trash_object 的注释)。
        let Some(server_id) = id.server_id() else {
            let hashed_id = id.uid();
            // Dais:本地对象 untrash —— 清 trashed_ts 同时把
        ...
        // Dais:云端 untrash RPC 已删除,这里折叠为本地直写并清 pending_untrash 位。
        ObjectStoreModel::handle(ctx).update(ctx, |object_store_model, ctx| {
            if let Some(object) = object_store_model.get_mut_by_uid(&hashed_id) {
                object.metadata_mut().trashed_ts = None;
                object
    ...
    pub fn delete_object_with_initiated_by(
        ...
        // Dais:云端 delete RPC 已删除,这里折叠为本地直接清除。
    ...
    pub fn empty_trash(&mut self, space: Space, ctx: &mut ModelContext<Self>) {
        // Dais:Empty Trash 走纯本地路径。原实现调用上游云端 empty_trash 接口,
...
// `update_object` Dais 本地化后,云端断言全部失效;本文件原属 Phase 2d-4a

...
///
/// Dais Wave 3-1:`AuthClient` trait 已物理删。Bearer token 来源改为直接读取
/// `AuthState::get_access_token_ignoring_validity()`(在 Dais 路径下仅在用户挂了
...
fn remote_server_identity_key(auth_state: &AuthState) -> String {
    // Dais 不再区分匿名 / 已登录身份,统一用 `user_id()`(本地测试 UID)。
    auth_state
        .user_id()

...
fn open_notebook(
    ...
    let load_future = handle.update(app, |view, ctx| {
        view.load(notebook, &DaisDriveObjectSettings::default(), ctx)
    });
...
fn test_close_unmodified() {
    ...
    App::test((), |mut app| async move {
        ...
        // Dais(Wave 4):SyncQueue 整删,原 stop_dequeueing + assert queue 长度不适用。
        ...
        app.read(|ctx| {
            ...
            // Dais(Wave 4):SyncQueue 整删,原 `sync_queue.is_empty()` 断言不适用。
        })
    });
}

...
// Dais Wave 3-1:`AuthClient` trait 与 `workspace:debug_create_anonymous_user`
// debug action 随 auth 子系统下线一同物理删。
use crate::app_state::get_app_state;
use crate::terminal::alt_screen_reporting::AltScreenReporting;
use crate::terminal::general_settings::GeneralSettings;
use crate::workspace::cross_window_tab_drag::CrossWindowTabDrag;
// Dais Wave 3-1:`ServerApiProvider` 不再被本文件使用,`debug_create_anonymous_user`
...
pub fn init_global_actions(app: &mut AppContext) {
    ...
    // Dais Wave 3-1:`workspace:debug_create_anonymous_user` global action 已随
    // 上游匿名用户创建云端入口一同物理删。
...
// Dais Wave 3-1:`fn create_anonymous_user` debug 调试入口已随上游匿名用户
// 创建云端入口一同物理删。Dais 已无匿名用户概念。

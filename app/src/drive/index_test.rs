...
fn test_retry_menu_item_logic() {
    App::test(Assets, |mut app| async move {
        ...
        // Dais(Wave 4):SyncQueue 整删,原本验证 SyncQueue 队列变化的
        ...
        // Dais(Wave 4):原验证 SyncQueue 队头是 CreateWorkflow,SyncQueue 整删后不适用。

        // Dais(Wave 4):原验证 SyncQueue 队列长度 + UpdateWorkflow tag,SyncQueue 整删后不适用。
    })
}

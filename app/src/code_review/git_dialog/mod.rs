...
// 收口说明:原 `should_send_git_ops_ai_request`(flag + per-feature AI 开关 +
// 企业检查)为全仓零调用死开关,承诺的 commit/PR AI 草稿链路从未实现
// (commit.rs 仅剩描述性注释,无任何 AI 请求设施),已删除。
// 待 ftr 落地真实请求链路后,在此恢复同形谓词并在 commit.rs 接线。

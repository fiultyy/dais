...
pub struct SkillManager {
    ...
    /// Skills bundled into Dais, each with activation condition and icon.
...
impl SkillManager {
    ...
    ///
    /// 同名多份时按 [`provider_rank`] 升序取第一(`Agents > Dais > Claude > …`),
    ...
    /// Load skill definitions bundled with Dais.
...
///
/// Supported variables:
/// - `{{warp_server_url}}` - Empty in Dais; retained for bundled skill compatibility.
...
/// branded icon; everything else falls back to the Dais logo.

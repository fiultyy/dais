...
fn test_harness_filter_matches_only_selected_harness() {
    App::test((), |mut app| async move {
        ...
        // Snapshot present but no harness set → Some(Oz), matches Dais Agent.
        ...
        // Local conversation: effectively Dais Agent.
        ...
        app.update(|ctx| {
            ...
            // Dais Agent / Oz → default-snapshot task and local conversation.
            ...
            assert_eq!(
                oz_items.len(),
                2,
                "expected 2 Dais Agent matches, got {oz_items:?}"
            );
            ...
            assert!(
                ...
                "stub task with no snapshot should not match the Dais Agent filter"
            );
        });
    });
}

...
impl FileBasedMCPManager {
    ...
    /// - For `MCPProvider::Dais`: the logical root for `~/.warp*/.mcp.json`.
        ...
        self.file_based_servers_by_root
            ...
            .any(|(root_path, provider_map)| {
                provider_map.iter().any(|(provider, hashes)| {
                    ...
                    match provider {
                        MCPProvider::Dais => Self::is_global_warp_root(root_path),
    ...
    /// Dais config (`~/.warp/.mcp.json`). Global Dais servers always auto-spawn.
    ...
    fn spawn_file_based_servers(
        ...
        // - Global Dais: always auto-spawn.
        // - Global non-Dais: auto-spawn iff the toggle is on.
    ...
    fn handle_file_based_mcp_enabled_change(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        // - Global Dais servers always spawn regardless of the toggle.
        ...
        if !AISettings::as_ref(ctx).is_file_based_mcp_enabled(ctx) {
            ...
            // Toggle on: spawn global third-party servers (global Dais servers are
    ...
    /// exception: global Dais installs are discovered in `~/.warp*/`, which
    ...
    pub fn spawn_root_for_installation(&self, uuid: Uuid) -> Option<PathBuf> {
        ...
        // Global Dais installs live under `~/.warp*/`, which is internal Dais
        // state rather than a meaningful working directory. Map them to the
        // home dir so all global installs (Dais and third-party) share a
        // consistent cwd.

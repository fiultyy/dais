...
pub const SERVER_OVERLOADED_TASK_FAILURE_MESSAGE: &str =
    "Dais is temporarily overloaded. Please try again shortly.";

/// JSON payload for starting an agent run. In Dais this is only used by local UI/CLI
...
impl AmbientAgentTaskId {
    /// Dais(本地化,Phase 3b-4):本地生成一个 UUID v4 作为 task_id,避免本地

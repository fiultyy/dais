//! Dais(Phase 3c 子任务 A1):本地化为永远"无限额"stub。
//!
//! 历史职责:warp.dev 服务端 RPC 驱动的"每月 AI 请求配额"模型。
//! Dais 走 BYOP(Bring Your Own Provider),用户自己付钱给 LLM 提供商,
...
///
/// Dais:本地化场景下永远不会触达(因 `ambient_only_credits_remaining` 恒为 `None`),
/// 仍保留常量定义以兼容外部 import。
...
/// 历史:服务端下发的"每月请求额度"快照。
/// Dais:仅作为类型壳保留(`AISettings::update_quota_info` / `ai_assistant/requests.rs`
...
/// Dais:仅作为类型壳保留(`ai_assistant/requests.rs` 仍会构造此类型)。
...
/// Dais:保留 enum 定义以兼容订阅回调 `match` 模式;
...
impl AIRequestUsageModel {
    ...
    /// Dais(本地化):永远返回 true,BYOP 本地运行不受云端限额约束。
    ...
    /// Dais(本地化):无云端计量,固定返回 0。
    pub fn requests_used(&self) -> usize {
        0
    }

    /// Dais(本地化):无云端计量,固定返回 0.0。
    pub fn request_percentage_used(&self) -> f32 {
        0.0
    }

    /// Dais(本地化):无云端 limit,固定返回 `usize::MAX`。
    pub fn request_limit(&self) -> usize {
        usize::MAX
    }

    /// Dais(本地化):远期 placeholder 时间。
    ...
    /// Dais(本地化):本地用户不存在 bonus grants。
    pub fn bonus_grants(&self) -> &[BonusGrant] {
        &[]
    }

    /// Dais(本地化):本地用户没有 ambient-only credits 概念。
    pub fn ambient_only_credits_remaining(&self) -> Option<i32> {
        None
    }

    /// Dais(本地化):本地用户没有 workspace bonus credits 概念。
    ...
    /// Dais(本地化):语音输入不受云端额度限制,永远返回 true。

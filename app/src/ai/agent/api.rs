...
pub struct RequestParams {
    ...
    /// Dais BYOP 专用:用户在 设置 → Agents → Rules 创建的全局 Rules
    ...
    /// Dais BYOP 专用:本地会话 id,只用于 request-readiness 诊断日志。
    pub byop_conversation_id: Option<AIConversationId>,
    /// Dais BYOP 专用:单次请求内的非持久诊断关联 id。
    ...
    /// Dais BYOP 专用:发起本请求时,关联的 LRC(Long Running Command)block id。
    ...
    /// Dais BYOP 专用:LRC 当前快照。`UserQuery.running_command` 只覆盖用户输入轮,
    /// auto-resume / tool result 后续轮需要通过这里继续携带最新 PTY 内容。
    pub lrc_running_command: Option<RunningCommand>,
    /// Dais BYOP 本地会话压缩 sidecar 快照(controller 把 conversation.compaction_state.clone() 塞进来)。
    ...
    /// Dais BYOP repair sidecar 快照。serializer 只读使用,不在请求构造中反序列化持久化 JSON。
    pub byop_repair_state: crate::ai::byop_readiness::RepairStateStatus,
    /// Dais BYOP 专用:本轮是否需要模拟上游 CreateTask 流程来升级 optimistic CLI subtask。
    /// 只有用户刚 tag-in 的首轮需要;已存在 CLI subagent 的后续轮只复用 task,不能重复 spawn。
    pub lrc_should_spawn_subagent: bool,
    /// Dais BYOP 专用:本轮响应应该写入的 task。普通对话是 root task;
...
impl RequestParams {
    ...
    pub fn new(
        ...
        // Dais BYOP 修复 Issue #116:gate 在 `is_memory_enabled`,具体收集逻辑

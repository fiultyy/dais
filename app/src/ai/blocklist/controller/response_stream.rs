...
fn byop_dispatch_info(
    ...
    // 任一模型不是 BYOP 编码(比如 fallback 到非 BYOP 默认),则跳过 — Dais 主路径都是 BYOP,
...
impl ResponseStream {
    ...
    /// Dais BYOP 本地会话压缩:返回本流是否在跑 SummarizeConversation,
...
async fn byop_required_response_stream(
    ...
    log::debug!("No BYOP provider selected for Dais agent request");

...
pub struct SessionContext {
    ...
    /// Dais:legacy SSH session(用户在本地 PTY 手敲 `ssh xxx@yyy`,
...
impl SessionContext {
    ...
    /// Dais:legacy SSH 连接信息(host/port),仅在 `is_legacy_ssh()` 为 true 时有意义。
    pub fn ssh_connection_info(&self) -> Option<&InteractiveSshCommand> {
        self.ssh_connection_info.as_ref()
    }

    /// Dais:本会话是否为 legacy SSH(用户手敲 ssh,远端无 warp hook)。
...
pub struct BlocklistAIController {
    ...
    /// conversation, because the ambient agent task driver owns the entire Dais window working on a task, and any
...
impl BlocklistAIController {
                ...
                error.downcast_ref::<crate::ai::agent::conversation::UpdateConversationError>()
            {
                if matches!(
                    ...
                    return self.complete_byop_blocked_request(
                        ...
                        request_params.model.clone(),
                        is_queued_prompt,
                        "Dais couldn't save the BYOP conversation state needed to send this \
    ...
    fn handle_response_stream_event(
        ...
        match event {
                    ...
                    Ok(event) => {
                        ...
                        match event {
                            ...
                            warp_multi_agent_api::response_event::Type::Finished(
                                ...
                                // Dais BYOP 本地会话压缩:在 stream finished 前拿 summarization 标志
            ...
            ResponseStreamEvent::AfterStreamFinished { cancellation } => {
                ...
                // Dais BYOP:收集本轮新加 message id,稍后用于在 EMPTY 分支检测
                ...
                if let Some(stream_cancellation) = &cancellation {
                    ...
                    // Dais:LRC tag-in 首轮自动授权 agent 工具执行。
                    //
                    ...
                    // Dais BYOP:from_args 解析失败时,chat_stream 走 fallback emit
                    ...
                    // 同标记反复命中导致死循环。
                    // Dais BYOP:没有进入 AIAgentAction 队列的 synthetic
    ...
    // Dais(Phase 3c A1):删除 `maybe_refresh_ai_overages` 函数。
    ...
    pub(super) fn handle_response_stream_finished(
        ...
        // Dais BYOP 本地会话压缩:在 token_usage move 进下面 closure 前先聚合,
        ...
        match finished_event.reason {
            Some(warp_multi_agent_api::response_event::stream_finished::Reason::Done(_)) | None => {
                ...
                // Dais BYOP 本地会话压缩 - auto overflow 触发(对齐 opencode `processor.ts:395-403`)
                // 仅在本流不是摘要本身时检查,防止递归。
...
/// Dais BYOP 专用:LRC tag-in / agent-monitored 场景下提取 RunningCommand。
///
/// 上游 `get_running_command` 在 `is_agent_monitoring()` 时返回 None — 因为 Dais 自家

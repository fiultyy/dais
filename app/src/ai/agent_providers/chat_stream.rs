...
/// Dais:渲染 SSH 会话状态块,append 到 system prompt 末尾。
///
...
fn build_user_message_with_binaries(
    ...
    for bin in binaries {
        if !caps.supports_mime(&bin.content_type) {
            // Dais 对齐 opencode `unsupportedParts`(packages/opencode/src/provider/transform.ts:305-341):
...
fn build_chat_request(
    ...
    // Dais:legacy SSH 会话画像补丁。`render_system` 走 AIAgentContext,
    ...
    // Dais BYOP 本地会话压缩:把 conversation.compaction_state 应用到 message 序列。
    ...
    // Dais:历史里被 skip 掉的 subagent ToolCall 对应的 call_id —— 它们的
    ...
    // Dais:仅在"同一 task_id 下真的落地过结构化 ToolCall"时才允许剥离 AgentOutput
    ...
    for (idx, msg) in all_msgs.iter().enumerate() {
        ...
        match inner {
            api::message::Message::UserQuery(u) => {
                flush_assistant_buffer(&mut buf, &mut messages, &mut outbound_tool_groups);
                // Dais:历史轮多模态保活。warp 自家路径靠云端 server 重注入 InputContext,
            ...
            api::message::Message::ToolCall(tc) => {
                // Dais BYOP:**虚拟 subagent tool_call 不发给上游模型**。
            ...
            api::message::Message::ToolCallResult(tcr) => {
                flush_assistant_buffer(&mut buf, &mut messages, &mut outbound_tool_groups);
                // Dais:对应 ToolCall 已被 skip(subagent 虚拟 call)→ result 也 skip,
    ...
    for input in &params.input {
        match input {
            AIAgentInput::UserQuery {
                ...
                // 与本路径不重叠。
                //
                // Dais：LRC tag-in 场景下，`running_command: Some(...)` 含完整 PTY 上下文
            ...
            AIAgentInput::SummarizeConversation {
                ...
                // Dais BYOP 本地会话压缩入口 — 1:1 对齐 opencode `compaction.ts processCompaction`。
                //
...
/// Dais Drive 本地文档存储(AIDocumentModel),不碰文件系统、不跑命令,语义上
...
fn build_tools_array(params: &RequestParams) -> Vec<GenaiTool> {
    // Dais A2:LRC tag-in 场景剔除 `run_shell_command`,迫使模型选 PTY 操作类工具。
    //
    ...
    let web_enabled = params.web_search_enabled;
    let plan_mode = is_plan_mode_turn(&params.input);
    // Dais BYOP:`suggest_prompt` chip UI 已通过 view 层订阅
    ...
    let mut out: Vec<GenaiTool> = tools::REGISTRY
        ...
        .filter(|t| {
            ...
            // suggest_new_conversation:无 UI 实现,executor 在 Dais 改为
...
pub(super) fn build_client(
    ...
    // Dais BYOP:SSE 流必须不带 gzip。`Accept-Encoding: gzip` 会让 nginx
    ...
    // 由入口 bin 注册:`bin/oss.rs` → "Dais";其它 channel 自带各自名称)。
...
/// - `Dais/<git-tag>` —— release 构建有 `GIT_RELEASE_TAG` 注入时
/// - `Dais` —— Dev / 本地构建无版本时
///
/// 应用名一律从 `ChannelState::app_id().application_name()` 取,确保与入口 bin
/// 注册的 `AppId` 一致(`bin/oss.rs` 注册 "Dais")。
...
/// 中转 / 本地服务 / 大部分国内云)一律不发。
///
/// Dais 这边没有 `providerID` 这个维度,只有用户自由填写的 `base_url`。
/// 因此通过 `base_url` 反推:
...
fn build_chat_options(
    ...
    //
    //    opencode 用 `providerID`(用户 config 选的字符串)做判定;Dais 这边没有
    ...
    //   需要显式 `extra_body.thinking.type=disabled` 才能关闭。Dais 本地 fork
...
pub async fn generate_byop_output(
    ...
    // Dais:请求 shaping(Anthropic cache_control / system-in-messages 布局)必须跟随
    ...
    // Dais:历史轮多模态保活。除 query 文本外,把当前轮 UserQuery.context 里的所有
    ...
    let stream = async_stream::stream! {
        ...
        if lrc_should_spawn_subagent {
            ...
            // c) Dais A1:把当前轮的 UserQuery 也复制一份到 subtask,初始化 subtask 的
        ...
        // Dais:content→tool 提取 fallback 只对 Ollama 生效,与 :1412 的历史过滤对称——
        ...
        for call in ordered_tool_calls {
            ...
            // Dais BYOP todowrite 拦截:不映射到 protobuf executor,合成
            ...
            // Dais BYOP web 工具拦截:webfetch / websearch 不映射到 protobuf
...
fn make_user_query_message(
    ...
    // Dais:把 multimodal binary(image / pdf / audio 等)写进 `UserQuery.context.images`
...
mod cache_boundary_stability_tests {
    ...
    fn build_three_turn_conversation() -> Vec<ChatMessage> {
        vec![
            ChatMessage::system(
                "You are a helpful coding assistant for Dais BYOP.\n\

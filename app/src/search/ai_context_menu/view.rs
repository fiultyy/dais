...
impl AIContextMenu {
    ...
    pub(crate) fn get_categories_for_mode(
        ...
        if is_cli_agent_input {
            ...
            // Dais:原会根据 outline_codebase_symbols_for_at_context_menu 设置 push Code
        ...
        if is_ai_or_autodetect_mode {
            ...
            // Dais:Code 分类随 outline 下线同步推退,不再 push。
            ...
            // Dais:原这里还会按 outline_codebase_symbols_for_at_context_menu push Code
            // 分类,现 outline 下线,Code 分类不再出现。
    ...
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        // Dais:原这里会创建 CodeSymbolCache(订阅 RepoOutlines) 以支持
    ...
    fn reset_mixer(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        match self.state.navigation_state {
            ...
            // Dais:Code 分类随 outline 下线推退。该分类不会出现在 categories 中,
    ...
    fn setup_data_sources_for_all_categories(&mut self, query: &str, ctx: &mut ViewContext<Self>) {
        ...
        for category in categories.iter() {
            match category {
                ...
                // Dais:Code 分类随 outline 下线推退;不会出现但保留 noop 分支
                // 避免 match 错误。
    ...
    // Dais:原 `render_code_symbols_indexing` 负责在代码符号索引中时渲染
    ...
    fn render_empty_state(
        ...
        // Dais:原这里会检查 Code 分类下是否正在索引代码符号,如是则走

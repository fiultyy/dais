...
3. Orchestration tab：runs → tasks（点击选中）→ task panel（Dispatch 按钮 + dispatch 明细：dispatch_contexts JOIN worker_dispatches）→ pending gates（选项 chip 一键 resolve / 自定义 resolution）→ 消息列表（点击选中→详情卡片：from/to/subject/type/priority/body）→ 消息 composer（send-message 子进程闭环）

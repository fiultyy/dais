...
| `start-worker` | `<task_id> [--command <cmd>] [--session <session_sid>]` | 为任务建 dispatch(worker_dispatch + dispatch_context 联动)+ **自动绑 pane(D-04)**:`--session` 显式绑定该 session 的 pane(失败=错误);无 `--session` 时 best-effort 绑当前活跃 pane(失败仅 stderr 提示)+ **绑定成功自动 mark-ready(D-18)**:worker starting→ready, dispatch pending→dispatched, task ready→dispatched——block 结算的前提(settle 要求 task+dispatch 双 dispatched,否则 InactiveDispatch 静默拒绝) | stdout: `ctx_<id>`(首行,兼容);绑定成功再加 `ctx_<id> assigned to ...` 与 `ctx_<id> ready (dispatch + task -> dispatched)` | zap 期不变;`--command`(block 驱动结算)为 6f08df11 加;`--session` 自动绑定 + assignee 落库为 D-04 修;自动 mark-ready 为 D-18 修 | agent_sdk / dispatch_assign.rs |
